...
class ExpDat():
    ...
    def get_subtracted_drug_data(self, trial_type):
        v_nd = self.trials['Pre-drug'][f'vcp_70_70']
        v_drug = self.trials['Post-drug'][f'vcp_70_70']

        data_nd = get_exp_as_df(self.h5file, v_nd, self.cm,
                is_filtered=True)
        data_drug = get_exp_as_df(self.h5file, v_drug, self.cm,
                is_filtered=True)
    ...
    def plot_paced_data(self, with_kernik=False, save_to=None, is_shown=True):
        fig, axs = plt.subplots(2, 1, sharex=True, figsize=(12, 8))
        i=0
        ...
        fig.suptitle("Paced Activity", fontsize=22)
        if save_to is not None:
            plt.savefig(f'{save_to}/paced.png')
    ...
    def plot_vc_data(self, with_kernik=False, is_shown=True):
        if is_shown:
            currents = input('Which currents do you want to consider? Separate them with a comma (I_Kr,I_CaL,I_Na,I_To,I_K1,I_F,I_Ks) ')
            each_curr = currents.split(',')
        ...
        if 'vcp_70_70' in self.trials['Washoff'].keys():
            trials.append(self.trials['Washoff']['vcp_70_70'])
            styles.append(['slategrey', '--', 'Wash-off'])
...
def get_apd(t, v, c, percent=90):
    ...
    dvdt_max_idx = didt_max_idx + np.diff(
            v[didt_max_idx:]).argmax()
    if v[dvdt_max_idx] < -65:
        dvdt_max_idx = np.diff(v).argmax()

    rmp = np.min(v)
    apa = np.max(v[dvdt_max_idx:]) - rmp
    ...
    #This APD is calculated from dv/dt max
    apd = t[apd_idx]
    return apd, apd_v
